...
const SidebarAddCategory = ({ open, toggle, onSuccess, selectedCategory }) => {
  // ** Hooks
  const {
    reset,
    control,
  ...
  useEffect(() => {
    if (selectedCategory) {
      setValue('name', selectedCategory.title)
    }
  ...
  const onSubmit = async (data) => {
    try {
      if (selectedCategory) {
        await updateCategorie({ id: selectedCategory.id, ...data })
      } else {
  ...
  return (
    ...
    >
      <Header>
        <Typography variant='h6'>{selectedCategory ? 'Modifier' : 'Ajouter'} une Catégorie</Typography>
